...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        var content = `${member} se ha unido al servidor.`, total = 0, regular = 0, _fake = 0, bonus = 0;
        if(invite == guild.vanityURLCode) content = settings.defaultMessage ? settings.defaultMessage : `-member- se une al servidor! Pero no conozco la invitación que se le ocurrió. : tada: `;
        else content = settings.welcomeMessage ? settings.welcomeMessage : `The -member-, se unió al servidor usando la invitación del -target-.`;
...
client.on("guildMemberRemove", (member) => {
    ...
    var total = 0, bonus = 0, regular = 0, fakecount = 0, channel = member.guild.channels.cache.get(settings.Channel), content = settings.leaveMessage ? settings.leaveMessage : `${member} Salió del Servidor..`, data = db.get(`invites.${member.id}`);
    if(!data){
        if(channel){
            content = content
            .replace("-member-", `${member}`);
