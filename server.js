...
client.on("message", async message => {
   let canal1 = client.channels.cache.filter(c => c.name == "eco");

  if (message.channel.name == "interchat-xd") {
    message.channel.send("Ese canal no es, cambia el nombre del canal `wolf`");
  }
  message.channel.createInvite({ maxAge: 0 }).then(link => {
    const embed1 = new Discord.MessageEmbed()
      ...
      //.setImage("https://i.pinimg.com/originals/94/db/94/94db94d97ee968cb3b7df65adab52f09.jpg")
    ...
    if (message.channel.name == "eco") {
      if (array2.some(word => message.content.toLowerCase().includes(word))) {
        message.delete();
        message
          .reply("No se permiten PACMANS ni LINKS")
          .then(response => {
       ...
       canal1.forEach(m => {
          try {
            client.channels.cache.get(m.id).send(embed1);
          } catch (e) {
          }
        });
      }
    if (!message.channel.name == "eco") return;
    if (message.channel.name == "eco") return message.delete();
    }
  });
})
