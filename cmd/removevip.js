...
if (!dev.includes(message.author.id)) return message.channel.send("❌ `|` **No tienes permisos Suficientes**")
    ...
    if (premium1.includes(user.id) == false)
      return message.channel.send("❌ `|` **Este usuario no pertenece a la base de datos de los Usuarios Vips del bot**")

    await vips_db.extract("Vips", user.id);
    message.channel.send("✅ `|` **Usuario eliminado correctamente en la base de datos de los Usuarios Vips del bot**")
  client.users.cache.get(user.id).send("❌ `|` **Has sido eliminado correctamente en la base de datos de los Vips del bot**").catch(e => {
