...
exports.run = async (client, message, args) => {
  ...
  if (db_marry.tiene(message.author.id))
    return message.channel.send("Tu ya estas casado con alguien."); // Envia este mensaje si la persona que propone matrimonio esta casada.
  ...
  collector.on("collect", collected => {
    if (collected.content.toLowerCase() === "si") {
      ...
      db_marry.establecer(message.author.id, {
        id: usuario.id,
        tag: usuario.username
      }) /
        db_marry.establecer(usuario.id, {
