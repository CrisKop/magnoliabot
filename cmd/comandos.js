...
let prefix_db = new db.crearDB("prefixes")
  ...
  if(args[0] === "staff") {
    ...
    .addField("Comandos/info:", "`botinfo` - `falexyinfo` - `listvips` - `afk` - `start-giveaway` - `reroll` - `end-giveaway` - `sugerencia`")
    message.channel.send(staff)
  }
   if(args[0] === "config") {
    const staff = new Discord.MessageEmbed()
  ...
  if(args[0] === "seguridad") {
    ...
    .addField("Comandos/Seguridad:", "`blockword` - `unlockword` - `wordslist` - `anti-loggers` - `anti-joins` - `anti-flood` - `falexydetectar` - `falexyban` - `snipe`")
  ...
  if(args[0] === "alianza"){
    ...
    .setThumbnail("https://cdn.discordapp.com/attachments/758040716340363385/759031207098318908/3468129.png")
    .setColor("RANDOM")
    .setAuthor(`Alianzas de ${client.user.username}`, client.user.displayAvatarURL())
    .addField("Comandos/Alianzas:", "`setchannel` - `points`")
    message.channel.send(se)
  }
  if(args[0] === "all"){
