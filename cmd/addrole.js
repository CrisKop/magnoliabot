exports.run = async (client, message, args) => {
  ...
  if (message.author == owner) {
    ...
    if (user.roles.cache.has(role.id))
      return message.channel.send("Ese usuario ya cuenta con ese rol");

    await user.roles.add(role.id);
    return message.channel.send(
      `El rol ${role.name} ha sido agregado correctamente al usuario ${user}`
    );
  }
  ...
  if (!user)
    return message.channel.send("Debes mencionar a un usuario válido."); // Si el usuario que menciono no existe devuelve ese mensaje.
  if (user == owner)
    return message.channel.send("No puedes añadir roles a ese usuario"); // Si el usuario mencionado es el dueño del servidor devuelve ese mansaje
  ...
  if (user == message.author.id) {
    ...
    if (!role.editable) return message.channel.send("Rol mas alto que el mio");
    if (user.roles.cache.has(role.id))
      return message.channel.send("Ya cuentas con ese rol");
  ...
  if (message.member.roles.highest.comparePositionTo(user.roles.highest) <= 0)
    return message.channel.send(
      "No puedes añadir roles a ese usuario, puede que tenga un rol igual o superior al tuyo."
    ); // Si el usuario mencionado tiene un rol igual o superior al del autor del mansaje devuelve esto
  ...
  if (role.comparePositionTo(message.member.roles.highest) >= 0)
    return message.channel.send(
      "No puedes añadir un rol igual o superior al tuyo."
    ); // Si el rol mencionado es igual o superior al del autor del mensaje devuelve esto
  ...
  if (user.roles.cache.has(role.id))
    return message.channel.send("Ese usuario ya cuenta con ese rol"); // Si el usuario mencionado ya tiene ese rol devuelve ese mansaje

  await user.roles.add(role.id); // Agrega el rol mencionado al usuario mencionado
  return message.channel.send(
    `El rol **${role.name}** ha sido agregado correctamente al usuario **${user}** | Dado por: **${message.member.displayName}**`
  ); // Envía un mensaje al canal diciendo que le agrego un rol al usuario mencionado, pueden poner un embed si quieren que sea vea mas bonito :)
};
